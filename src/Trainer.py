...
class Trainer():
    ...
    def train(self):
        ...
        train_loss = 0
        for fixed_image, moving_image, fixed_image_mask, moving_image_mask in tqdm(self.trainloader):
            ...
            moving_image_mask = moving_image_mask.to(self.model.device)
            self.model.optimizer.zero_grad()
            pred_image, flow_field = self.model.net(moving_image.float(),fixed_image.float())
            pred_mask = self.model.net.transformer(moving_image_mask.float(), flow_field)
            loss, train_mean_dice = self.model.criterion(pred_image, pred_mask.detach().cpu(), fixed_image, fixed_image_mask, flow_field)
            loss.backward()
            self.model.optimizer.step()
            train_loss += loss.item()

        return train_loss, train_mean_dice
    ...
    def test(self):
        ...
        test_loss = 0
        with torch.no_grad():
            for fixed_image, moving_image, fixed_image_mask, moving_image_mask in tqdm(self.testloader):
                fixed_image, moving_image = fixed_image.to(self.model.device), moving_image.to(self.model.device)
                #fixed_image_mask, moving_image_mask = fixed_image_mask.to(self.model.device), moving_image_mask.to(self.model.device)
                moving_image_mask = moving_image_mask.to(self.model.device)
                pred_image, flow_field = self.model.net(moving_image.float(),fixed_image.float())
                pred_mask = self.model.net.transformer(moving_image_mask.float(), flow_field)
                loss, test_mean_dice = self.model.criterion(pred_image, pred_mask.detach().cpu(), fixed_image, fixed_image_mask, flow_field)
                test_loss += loss.item()

        return test_loss, test_mean_dice
    ...
    def update_state(self):
        train_loss, train_mean_Dice = self.train()
        test_loss, test_mean_Dice = self.test()
