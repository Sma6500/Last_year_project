...
class Model: #very basic, need inheritage later and a more complexe build, but it's not necessary right now
    ...
    def criterion(self, pred_image, pred_mask, fixed_image, fixed_mask, flow_field):
        ...
        #Dice
        dices=compute_dice(pred_mask, fixed_mask)
        mean_dice=torch.mean(dices)
        if self.criterion_config['Dice'] is not(None):
            #comme on veut minimiser la loss généralement c'est probablement pas + mais - ^^'
            loss+=mean_dice*self.criterion_config['Dice']
